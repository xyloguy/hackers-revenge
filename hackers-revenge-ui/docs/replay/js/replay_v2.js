...
const P2_COLOR = "#ff0505";
const P1_HILIGHT_COLOR = "#0000ff";
const P2_HILIGHT_COLOR = "#aa0000";
...
var FULL_CANVAS_SIZE = 900;
var CANVAS_SCALE = 1.0;
// defaults but will be scaled to fit.
var CANVAS_SIZE = FULL_CANVAS_SIZE * CANVAS_SCALE;
var SQUARE_SIZE = CANVAS_SIZE/16;
var CELL_BUFFER = CANVAS_SIZE/128;
...
function Replayer(replay, next) {
    ...
    replayer.animate_current_cycle = function(cycle_length) {
        ...
        ctx.clearRect(0, 0 + SQUARE_SIZE, CANVAS_SIZE+SQUARE_SIZE, CANVAS_SIZE+(SQUARE_SIZE/2));
        replayer.draw_mem();
        var cl = cycle_length;
        ...
        ctx.fillRect(p1_cl.x, p1_cl.y + SQUARE_SIZE, SQUARE_SIZE-CELL_BUFFER, SQUARE_SIZE-CELL_BUFFER);
        ctx.globalAlpha = 1-alpha;

        ctx.fillStyle = P2_HILIGHT_COLOR;
        ctx.fillRect(p2_cl.x, p2_cl.y + SQUARE_SIZE, SQUARE_SIZE-CELL_BUFFER, SQUARE_SIZE-CELL_BUFFER);
        ctx.globalAlpha = alpha;
        ...
        if ("read_addr_last" in cj && cj.read_addr_last != null) {
            ...
            for (var a = 0 ; a < len ; a++) {
                ...
                ctx.fillRect(l.x, l.y + SQUARE_SIZE, SQUARE_SIZE-CELL_BUFFER, SQUARE_SIZE-CELL_BUFFER);
            }
            ...
            ctx.beginPath();
            ctx.moveTo(src.x + SQUARE_SIZE*0.5, src.y + (SQUARE_SIZE*0.5) + SQUARE_SIZE);
            ctx.lineTo(l.x + SQUARE_SIZE*0.5, l.y + (SQUARE_SIZE*0.5) + SQUARE_SIZE);
            ctx.stroke();
            ctx.restore();
            ctx.globalAlpha = alpha;
        ...
        if ("writes" in cj) {
            ...
            for (var wi = 0 ; wi < cj.writes.length; wi++) {
                ...
                pos.y += (SQUARE_SIZE*0.5) + SQUARE_SIZE;
    ...
    replayer.draw_code = function(is_tied) {
        ...
        if (status !== 0) {
            ctx.globalAlpha = .8;
            ctx.fillStyle = "#000";
            ctx.fillRect(SQUARE_SIZE, CANVAS_SIZE * .25 + SQUARE_SIZE, CANVAS_SIZE - SQUARE_SIZE * 2, CANVAS_SIZE / 2);
            ctx.globalAlpha = 1;
            ctx.lineWidth = 4;
            ...
            ctx.strokeRect(SQUARE_SIZE, CANVAS_SIZE * .25 + SQUARE_SIZE, CANVAS_SIZE - SQUARE_SIZE * 2, CANVAS_SIZE / 2);
            ctx.save();
        ...
        ctx.lineTo(SQUARE_SIZE*4.5, SQUARE_SIZE*.85);
        ctx.stroke();
    ...
    replayer.draw_mem = function() {
        ...
        for (var y = 0 ; y < 16 ; y++) {
            for (var x = 0 ; x < 16 ; x++) {
                ...
                var sy = (y * (SQUARE_SIZE) + 1) + SQUARE_SIZE;
...
document.addEventListener('replay_done',
    function (ev) {
        ...
        if (next_replay === undefined) {
            ...
            R = Replayer(next_replay, function () {
                trigger('replay_done');
            });
            R.run();
        }
...
document.addEventListener('run_queue_empty', function (ev) {
    RUN_QUEUE  = [];
    $.ajax({
        cache: false,
        dataType: 'json',
...
if (test == null) {
    ...
    $.when.apply(undefined, ajaxes).then(
        ...
        function () {
            alert("Failed to load specific battle, defaulting to all");
